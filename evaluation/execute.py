                            ...
                            try:
                                ...
                                if not file_content_full.strip() or (
                                    ...
                                    error_dict["messages"][msg_idx][
                                        "content"
                                    ] = f"Error: Referenced file '{file_ref_name}' was empty or had no instances."
                                    ...
                                    for instance_str in instances_text:
                                        ...
                                        if (
                                            not cleaned_instance_str
                                        ):  # Skip empty instances resulting from split
                                            continue
                                        ...
                                        instance_specific_dict["messages"][msg_idx][
                                            "content"
                                        ] = cleaned_instance_str
                                ...
                                error_dict["messages"][msg_idx][
                                    "content"
                                ] = f"Error: Referenced file '{file_ref_name}' not found."
                                resolved_dicts_for_this_variant.append(error_dict)
                                variant_had_file_expansion = (
                                    True  # Mark as "handled" to avoid falling through
                                )
                                break
                            except Exception as e:
                                ...
                                error_dict["messages"][msg_idx][
                                    "content"
                                ] = f"Error processing referenced file '{file_ref_name}': {e}"
                                resolved_dicts_for_this_variant.append(error_dict)
            ...
            for conv_dict in potential_conversation_dicts:
                try:
                    ...
                    if prompt:  # If a system prompt is provided, add it to the beginning of messages
                        ...
                        if (
                            conversation_obj.messages is None
                        ):  # Should be initialized by Pydantic if type is List[Message]
...
def main():
    ...
    print(
        f"Enviando payload a Supabase (HTML embebido, tamaño aproximado: {len(html_content_str)/1024 if html_content_str else 0:.2f} KB)..."
    )
