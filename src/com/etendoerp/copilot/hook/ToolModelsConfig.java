...
public class ToolModelsConfig implements CopilotQuestionHook {
   ...
   *
   * @param app The copilot app to process, including its tools and team members
   * @param json The JSON object to which the tool configurations will be added.
   *             Must contain an "extra_info" object.
   * @throws OBException if there's a critical error processing the hook
   */
  @Override
    ...
    try {
      ...
      for (CopilotApp copilotApp : set) {
        if (log.isDebugEnabled()) {
          log.debug(String.format("Adding tool models config to app: %s", copilotApp.getName()));
   ...
   *
   * @param app The copilot app to check (not used in this implementation)
