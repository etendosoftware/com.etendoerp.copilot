...
def _get_param_type_annotation(field_info) -> str:
    ...
    try:
        if hasattr(field_info, "annotation") and field_info.annotation:
            if hasattr(field_info.annotation, "__name__") and (field_info.annotation.__name__ != "Union"):
...
async def setup_agent_from_etendo(identifier: str, etendo_token: str) -> bool:
    """
    ...
    try:
        ...
        agent_config = await fetch_agent_structure_from_etendo(identifier, etendo_token)
        ...
        if agent_config.system_prompt:
            copilot_debug(
                f"System prompt configured for agent {identifier}: {agent_config.system_prompt[:100]}..."
            )
...
async def _get_agent_structure_tool() -> dict:
...
async def initialize_agent_from_etendo(identifier: str):
    ...
    try:
        ...
        success = await setup_agent_from_etendo(identifier, etendo_token)
