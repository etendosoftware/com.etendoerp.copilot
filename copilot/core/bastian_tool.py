...
class BastianFetcher(Tool):
    ...
    description = (
        "This is a tool which knows a lot about Etendo ERP. "
        "It takes a question and returns an answer."
    )
...
class XMLTranslatorTool(Tool):
    ...
    description = "Una herramienta que traduce texto en inglés dentro de propiedades XML a español."
    inputs = ["question"]
    ...
    def split_xml_into_segments(self, content, max_tokens):
        ...
        for child in root:
            if ET.tostring(current_segment).strip().decode() == "<{}></{}>".format(root.tag, root.tag):
                continue
