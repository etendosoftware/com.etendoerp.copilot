...
const ConversationsSidebar: React.FC<ConversationsSidebarProps> = ({
  ...
  const getConversationTitle = (conversation: IConversation) => {
    ...
    if (generatingTitles?.has(conversation.id)) {
      // display-friendly (localized) generating text with fallback to the existing Spanish text
  return <span className={`text-black italic ${fontWeight}`}>{labels?.ETCOP_GeneratingTitle ?? 'Generating...'}</span>;
    }

    if (conversation.title && conversation.title.trim() !== '') {
      // Detect temporary/current conversation titles so we can style them as temporary
  ...
  // display-friendly untitled (localized) with Spanish fallback
  ...
  const getTooltipTitle = (conversation: IConversation) => {
    if (generatingTitles?.has(conversation.id)) {
      // tooltip/technical text - use label if present, fallback to English string
    ...
    // tooltip fallback (English) to match previous behavior
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-50 h-full">
      ...
      <div className="flex-shrink-0 p-3 border-b border-gray-300">
        {/* Using the project's Button component for consistent styling. Import will be added by you. */}
        <Button
          fontSize={16}
